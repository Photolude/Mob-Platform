...
public class PluginDomain implements IPluginDomain {
	...
	public PluginArt getArt(String userToken, String role, String artPath) {
		if(StringUtils.isNullOrEmpty(userToken) || StringUtils.isNullOrEmpty(role) || StringUtils.isNullOrEmpty(artPath))
		{
			return null;
		}
		
		PluginArt retval = this.dataAccessLayer.getArt(userToken, role, artPath);
		
		if(retval != null && retval.getData() != null)
		{
			String content = retval.getData().substring(retval.getData().indexOf(',') + 1);
	...
	@Override
	public PluginDefinition getPluginForRole(String userToken, String role) {
		if(StringUtils.isNullOrEmpty(userToken) || StringUtils.isNullOrEmpty(role))
		{
			return null;
		}
		
		PluginDefinition definition = this.dataAccessLayer.getPluginByUserAndRole(userToken, role);
		
		if(definition != null)
		{
			definition.setIcon(null);
			definition.setServiceAliases(null);
